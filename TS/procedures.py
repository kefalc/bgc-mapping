...
from utils import softplus, sigmoid, det, BFGS_training, get_window, save_predictGP_dask
...
def pipeline(x_TS, y_TS, x_BGC, y_BGC, 
    ...
    try:
        ########## Here figure out how to make x-test have multiple entries --> should look like x_TS --> then just need to call predict once!
        maptimes = jnp.array([[0., 0., x] for x in estimtimes])   
        
        #mu = np.zeros(len(estimtimes))
        #var = np.zeros(len(estimtimes))
        
        mu, var = predict(params, x_BGC, y_BGC, x_TS, y_TS, maptimes, 
...
def task(center, return_params, lat_TS, lon_TS, t_TS, y_TS,
         ...
         correlated_nugget=False, nugget_prime=False, winsize_TS=10., winsize_BGC=10., estimtimes=None, verbose=False):
...
def gridded_tasks(lat_grid, lon_grid, depth, filename, ocean_mask, return_params=False, 
                  ...
                  winsize_TS=10., winsize_BGC=10., estimtimes=None, verbose=True):
    ...
    for ilat, lat in tqdm(enumerate(lat_grid)):
        for ilon, lon in tqdm(enumerate(lon_grid)):
            res = task((lat, lon), return_params, **data, return_estim=return_estim, correlated_nugget=correlated_nugget,
                       nugget_prime=nugget_prime, winsize_TS=winsize_TS,  winsize_BGC=winsize_BGC,  estimtimes=estimtimes, verbose=verbose)
...
def gridded_tasks_dask(client, outfile, filename, ocean_mask, return_params=False, 
                  correlated_nugget=False, nugget_prime=False, return_estim=True, 
                  winsize_TS=10., winsize_BGC=10., estimtimes=None, verbose=False):
    '''extract raw data from .npz file (filename) generated from generate_data and perform modelling tasks on given 
    grid points defined by lat_grid and lon_grid sequentially using dask
    ...
    data = jnp.load(filename)
    lat_TS = client.scatter(data['lat_TS'], broadcast=False)
    lon_TS = client.scatter(data['lon_TS'], broadcast=False)
    t_TS = client.scatter(data['t_TS'], broadcast=False)
    y_TS = client.scatter(data['y_TS'], broadcast=False)
    lat_BGC = client.scatter(data['lat_BGC'], broadcast=False)
    lon_BGC = client.scatter(data['lon_BGC'], broadcast=False)
    t_BGC = client.scatter(data['t_BGC'], broadcast=False)
    y_BGC = client.scatter(data['y_BGC'], broadcast=False)

    #print('data scattered')
    #print(client)
    
    mask0 = ocean_mask.float_mask.to_numpy()
    long = ocean_mask.lon.to_numpy()
    latg = ocean_mask.lat.to_numpy()
    lonM, latM = jnp.meshgrid(long, latg)
    lons = ma.MaskedArray(lonM, 1-mask0).compressed()  # 1-mask0 since ma.MaskedArray masks wanted (unwanted) values as False (True).
    lats = ma.MaskedArray(latM, 1-mask0).compressed()  # 1-mask0 since ma.MaskedArray masks wanted (unwanted) values to False (True).
    
    list_of_submitted_tasks = []
    #t_start = Time()
    iM, jM = np.where(ocean_mask.float_mask)
    for i, j, lat0, lon0 in zip(iM, jM, lats, lons):
        submitted = client.submit(task, 
                (lat0, lon0), return_params, lat_TS, lon_TS, t_TS, y_TS, lat_BGC, lon_BGC, t_BGC, y_BGC, 
                return_estim=return_estim, correlated_nugget=correlated_nugget, nugget_prime=nugget_prime, 
                winsize_TS=winsize_TS,  winsize_BGC=winsize_BGC,  estimtimes=estimtimes, verbose=False, 
                pure = False,
                workers = None 
        )
        list_of_submitted_tasks.append( submitted )
    res = client.gather(list_of_submitted_tasks)
    print('Dask computations done.')
    #print(res)
    
    save_predictGP_dask( res, ocean_mask, latg, long, estimtimes, outfile, winsize_TS, winsize_BGC, correlated_nugget=correlated_nugget)
    print('Covariance-parameters saved to outfile.')
    del list_of_submitted_tasks, submitted, lat_TS, lon_TS, t_TS, y_TS, lat_BGC, lon_BGC, t_BGC, y_BGC

    ###### Here, save file to netcdf format so that it can be opened with xarray.
    # if return_estim:
    #     estim = np.zeros((len(lat_grid), len(lon_grid), len(estimtimes)))
    #     var = np.zeros((len(lat_grid), len(lon_grid), len(estimtimes)))
    # beta = np.zeros((len(lat_grid), len(lon_grid)))
