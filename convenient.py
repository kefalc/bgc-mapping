...
def get_betaparams_2vars(betaparamfile) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray, zarr.hierarchy.Group):
    b = xr.open_dataset(betaparamfile)
    return b.latitude[:], b.longitude[:], b.betaparam_temp[:,:,:,0], b.betaparam_sal[:,:,:,0], b

def get_TSdata(datafile, plevel) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, xr.core.dataset.Dataset):
    d0 = xr.open_dataset(datafile).isel(pressure=plevel)
    time = (
        ( d0.time.values.astype('datetime64[ns]') - np.datetime64('1970-01-01T00:00:00', 'ns') 
       ) / np.timedelta64(1,'ns') 
    ) / 1e9 / 86400 # days ref. 1970-01-01
    
    temp0 = d0.temperature.values
    salt0 = d0.salinity.values
    return d0.lat.values, d0.lon.values, time, temp0, salt0, d0
